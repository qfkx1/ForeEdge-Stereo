...
def train(args):
    model = nn.DataParallel(ForeEdgeStereo(args))
    ...
    while should_keep_training:
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                save_path = Path(args.logdir + '/%d_%s.pth' % (total_steps + 1 + 0, args.name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='0')
    parser.add_argument('--restore_ckpt', default=None)
    parser.add_argument('--logdir', default='./checkpoints/0')
    parser.add_argument('--train_datasets', nargs='+', default=['sceneflow'],choices=['sceneflow', 'kitti', 'middlebury','eth3d','all'])
    ...
    parser.add_argument('--train_iters', type=int, default=22,help="number of updates to the disparity field in each forward pass.22")
    ...
    parser.add_argument('--saturation_range', type=float, nargs='+', default=[0, 1.4], help='color saturation[0, 1.4]')
