...
from core.foreedge_stereo import ForeEdgeStereo, autocast
...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    f = open('kitti15_noc2.txt', 'a')
    f.write("Validation kitti15: %f, %f\n" % (epe, d1))
...
@torch.no_grad()
def validate_middlebury(model, iters=32, mixed_prec=False):
    """ Peform validation using the Middlebury-V3 dataset """
    model.eval()
    aug_params = {}
    val_dataset = datasets.Middlebury(aug_params)
    ...
    return {'middlebury-epe': epe, 'middlebury-d1': d1}


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='./weights/sceneflow.pth')
    ...
    if args.restore_ckpt is not None:
        ...
        checkpoints = sorted(glob(osp.join(args.restore_ckpt)))
        for i in range(len(checkpoints)):
            checkpoint = torch.load(checkpoints[i])
            model.load_state_dict(checkpoint, strict=True)
            logging.info(f"Done loading checkpoint"+checkpoints[i])
            ...
            if args.dataset == 'eth3d':
                ...
                validate_middlebury(model, iters=args.valid_iters, mixed_prec=use_mixed_precision)
